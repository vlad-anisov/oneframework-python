...
его все три привязки одинаково, и никто не порождает.

До 02.09.2026 порождал питон -- интроспекцией своих же классов полей. Значит
завести тип поля можно было только правкой питона, а Kotlin и JavaScript
оставались читателями договора, который пишет третий: они не беднее по коду, у
них просто не было права голоса. Теперь источник -- сам файл, а привязки
сверяются с ним (``tests/test_protocol.py``).
"""
