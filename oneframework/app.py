...
class App:
    ...
    #: ``start()`` поднимал питоновский рантайм. Его нет; кадр спрашивают у
    #: устройства (`tests/parity/rt_host.mjs`).
    ...
    #: ``attach_logic()`` поднимал питоновский хост логики. Подключает её
    #: теперь `web/src/runtime/worker.js::attachLogic` -- из той же базы.
...
#: ``publish`` раскладывал приложение по базе. Делает это сборщик на JS: питон
#: говорит, что класть, кладёт тот же код, каким база живёт на устройстве.
