...
def project_root() -> Path:
    ...
    `package.json`, `vite.config.js`. Раньше искалось «над питоновским
    пакетом» -- это верно ровно в одном дереве, где всё лежит вместе. Разложи
    ядро и привязку по репозиториям, и поиск упирается в `pyproject.toml`, над
    которым никакого `web/` нет.
...
def write_app_db(app, seed_fn, out: Path, поверх: bool = False):
    ...
    if not поверх and out.exists():
        out.unlink()
    # План строит ядро: писателю едет **пакет объявления**. Питоновская копия
    # плана (`cli/plan.py`) удалена 21.08.2026 -- правило жило в двух местах и
    # держалось сверкой, а сверять стало нечего, когда сборка перешла на ядро.
...
#:
#: * ``copy_pyodide`` -- рантайм питона в сборку. Кладёт `cli/python_runtime.py`
#:   вместе с колёсами, одной работой вместо двух;
#: * ``forget_device_identity`` -- стереть `hlc:node` и `hlc:last` из собранной
#:   базы. Делает сборщик на JS, до выгрузки байтов, а не открывая файл заново;
#: * ``build_bundle`` -- пакет объявления файлом. Собирается по общему плану.
