...
def model_schema(model):
    ...
    ``label`` -- подпись модели для человека. Раньше её здесь не было, и это
    была настоящая дыра: документ объявлял себя пригодным для другого бэкенда,
    а тот не мог узнать, как называется модель на экране. Пока приложение
    собирал только питон, подпись бралась с класса и потери не было видно.
    """
