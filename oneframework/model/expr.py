...
#: ``evaluate`` жил здесь -- вычисление условия на записи обычным питоном.
#: Вычисляет его устройство (`libs/js/src/core/expr.js`), и питоновский был вторым:
#: видимость решал один, а `inspect --why` объяснял другим. Объяснение не тем
#: вычислителем -- худший род объяснения: оно выглядит ответом.
#:
#: Перед удалением оба сверены на корпусе `test_dsl`. Разошлись ровно в одном:
#: **пропущенный ключ записи**. У устройства он значит «не выбрано» и расширяет
#: условие, у питона значил «нет значения». Случай недостижим: колонка приходит
#: из базы всегда, пустая приезжает как `null`, и на `null` стороны отвечали
#: одинаково.
