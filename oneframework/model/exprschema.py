...
двум вычислителям. Опиши её прозой в каждом, и цена будет не путаница в чтении,
а молчание: узел, которого чужая библиотека не знает, доезжает не ошибкой, а не
тем условием -- то есть не тем списком на экране. Заметить можно только по
чужой жалобе.
...
**Каркас его не ввозит, и это не повод его удалить.** Он порождает
`protocol/expression.json` -- договор, который читают привязки, драйвер
грамматики и сюита; договор пересобирается командой, а не на запуске. Удалить
модуль значит вернуть форму узлов в прозу каждого языка.
