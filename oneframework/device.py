...
def action(fn=None, *, writes=None, label=None, name=None, args=None,
    ...
    Раньше это звалось ``on_device``, и имя называло место исполнения. Место
    здесь одно: считает устройство, другого исполнителя нет. Имя, сообщающее
    единственно возможное, не сообщает ничего -- зато закрывает собой то, чем
    метод является: действием над записями.
