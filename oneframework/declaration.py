...
``tests/together/test_declaration.py``.
...
def declare(app, seed=None) -> dict:
    ...
    Знание записано, а не получено вопросами: записанное умеет порождать любой
    язык, а спрашивать вызовами умеет только питон.

    ``seed`` -- функция демо-данных (``seed.py`` рядом с приложением). Она
    **прогоняется здесь**, и в пакет едут уже строки: прогоняй её сборка, пакет
    был бы беднее приложения, и собрать то же самое без питона стало бы нельзя.
    """
...
def record_seeds(app, seed=None):
    ...
    непустой (у пользователя она уже есть), и
...
class Bundle:
    ...
    def __init__(self, doc, *, source=None):
        ...
        # обещал бы одно, а сборка требовала другое, и пакет без раздела
        # проезжал бы молча.
    ...
    #: Совпадение описаний сторожит ``tests/together/test_protocol.py``. Второй записи
    #: правила создания таблиц здесь быть не должно.
