...
def fingerprints(bundle):
    ...
    """
    #: Считает отпечатки та сторона, что считает их на устройстве: сравнивай
    #: мы языки отпечатками своей копии -- сравнивали бы не тем, чем их
    #: различают на самом деле.
...
def test_the_rich_app_is_the_same_on_kotlin():
    ...
    Прежде привязка Kotlin объявляла три рода узлов из семнадцати, и
