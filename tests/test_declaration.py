...
def test_declaring_an_app_does_not_raise_the_runtime():
    ...
    Прежде объявление приложения поднимало весь питоновский рантайм:
    ...
    сам рантайм удалён, и первая половина списка запретов исчезла
...
def test_a_missing_section_is_refused_by_name():
    ...
    Раздел ``seeds`` появился, когда посев переехал в пакет. Пока
