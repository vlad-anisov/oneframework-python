...
def видно(узел, запись, состояние=None):
    ...
    Про пропущенный ключ записи проверка не спрашивает: на устройстве такой
    случай недостижим -- колонка приходит из базы всегда, а пустая приезжает
    как `null`. Спрашивается `null`.
    """
    если = узел.visible
