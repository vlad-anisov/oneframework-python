...
#: Ядро зовётся отсюда, а не из привязки. Правило простое: питон печатает
#: пакет, всё остальное делает ядро -- и обратного вызова из привязки в ядро
#: быть не должно. Пока он был (`cli/plan.py`, `cli/assets.py`, `core.py`),
#: привязка знала, где ядро лежит и как его запускать, а держалось это знание
#: одними проверками. Проверкам оно и принадлежит.
def _ядро(*части) -> Path:
    return ROOT.joinpath("libs", "js", *части)


def собрать_базу(пакет, куда, поверх=False):
    """Пакет -> файл SQLite. Пишет ядро -- тем же писателем, что и сборка.

    ``поверх`` -- открыть существующий файл, а не начать с чистого. Умолчание
    обратное, и не случайно: собранный подряд второй пример ложился поверх
    первого, база копила модели всех приложений сразу, и приложение на
    устройстве видело чужие экраны. Сборка при этом проходит молча.
    """
...
#: собирает файл (`собрать_базу` ниже).
...
#: Ядро рядом или его нет.
#:
#: В монорепозитории оно рядом; в расколотом `oneframework-python` -- нет, и
#: проверке, которой нужна работающая половина ядра, там взяться неоткуда.
#: Такие проверки **пропускаются с причиной**, а не падают: красное, которое
#: нельзя починить, приучает не читать вывод.
#:
#: Не путать с `tests/together/`. Туда уезжает файл, чей предмет целиком --
#: согласие сторон; такому в привязке делать нечего вовсе. Здесь другое: файл
#: про саму привязку, у которого две-три проверки просят ядро как обстановку.
...
#: Питоновские `cli/kotlin_app.py` и `cli/teavm.py` удалены: под
...
def план(пакет):
    ...
    Питоновская копия (`oneframework/cli/plan.py`) удалена: правило
