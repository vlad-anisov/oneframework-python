...
не нужная, чтобы собрать приложение. Удалён: то же самое умеет
...
#: Оснастка разработчика. Пусто -- отладчик удалён; множество
...
# ядро на JavaScript
# --------------------------------------------------------------------------
#: Сборщик переехал на JavaScript. Смысл переезда в одном: собрать
...
def test_the_javascript_core_is_self_contained():
    ...
    Безголовый рантайм переехал в `libs/js/src/core/` именно
    поэтому.
