...
def test_the_example_declares_logic_and_it_reaches_the_database(app, runtime):
    ...
    Сегодня модуля у примера **нет вовсе**. Оба действия, ради которых он
    ...
    # выкладку модулей в базу и удалён -- к тому дню он состоял из
...
def test_a_computed_field_has_no_column(app):
    ...
    # Сегодня поле объявлено **объявлением**, а не именем действия: раньше
